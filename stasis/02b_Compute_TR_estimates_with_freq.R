...
data_populations0 <- subset(final_export,data_group_name%in%paste0("B30",1:9) & population2=="pops" & tstrsplit(pop_label,"_")[[2]]=="herm")

data_populations=rbind(data_populations0, data_MD, data_WI)
dim(data_populations) #174 measurements
...
## Fit a linear mixed-effects model to estimate the mean value for each population
...
#### Estimate betas for sequential timepoints
...
for(i in 1:length(vect_P_traits2)){
  ...
  vA1 <- c("OF5","A0","A110","A130","A160","A1100")
  va1_gen=c(-5,0,10,30,60,100)
  ...
  for(temp_i in 2:3){
    va1_gen = c(0,10,30,60,100) 
    vA1 = paste0("A", temp_i,va1_gen)
    vA1[1] = "A0"
    temp_P = cbind(mean_P[pop_lev%in%vA1], va1_gen[order(vA1)])
    temp_P = temp_P[order(temp_P[,2]),]
  ...
  ### CA populations
  for(temp_i in 1:3){
    va1_gen = c(0,5,10,36,50,68,100)
    vA1 = paste0("CA", temp_i,va1_gen)
    vA1[1] = "A6140"
    va1_gen= va1_gen+140
...
save(list=ls(),file="~/PATH/TO/DIR/Cemee_Pop_WI/Analysis_Cemee_Pop_WI_betas_with_freq.RData")
