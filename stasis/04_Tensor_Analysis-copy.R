...
# Here we save a file that could be used on a HPC machine to compute the randomized
# eigentensors that are computationally demanding.
...
run_parallel_MCMC <- function(i){
  ...
  ## Here we compute the random Garrays using the Morrissey technique and save them in a list
  ...
  for(k in 1:7){
    ...
    rand.model_MCMC.temp <- MCMCglmm(cbind(c(T12, T13, T21, T23, T31, T32)) ~  trait - 1, random = ~us(trait):pop_label,
                                     rcov = ~us(trait):units, family = rep("gaussian", nb_trait), 
                                     data = ped_lines_current, prior = prior_mod, verbose = FALSE,nitt=150000, burnin=50000,thin=100)
...
plot((1:nnonzero)-0.2,unique(MCMC.covtensor$tensor.summary[1:(n*nnonzero),1]),
     xlab="",ylab=expression(alpha),pch=16,cex=1,xaxt="n",frame.plot=F,
     xlim=c(0.5,6.5),ylim=c(0,max(HPD.eT.val)),type="n")
...
# Now to compute confidence intervals for the correlations
