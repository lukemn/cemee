...
Y_temp=permuted_Y[,i]
mod_lm <- lm(Y_temp~1+(T12+T13+T21+T23+T31+T32)^2-(T12+T13+T21+T23+T31+T32)+ I(T12^2)+ I(T13^2)+I(T21^2)+ I(T23^2)+I(T31^2)+ I(T32^2), data = final_fertility2)

post_mod_gamma_Rdm_mvn =cbind(post_mod_gamma_Rdm_mvn ,mod_lm$coef[2:22])
}

all_RDM_EV_MVN=apply(post_mod_gamma_Rdm_mvn,2, extract_gamma_RV)


model_MCMC <- MCMCglmm(Y2 ~1+(T12+T13+T21+T23+T31+T32)^2-(T12+T13+T21+T23+T31+T32)+ I(T12^2)+ I(T13^2)+I(T21^2)+ I(T23^2)+I(T31^2)+ I(T32^2), data = final_fertility2 ,verbose = TRUE,nitt=1000000, burnin=500000,thin=50)
...
post_dist_all <-HPDinterval(model_MCMC $Sol,prob=.9)[2:22,]
...
plot(post_mod_gamma,c(21:1),yaxt="n",bty="n",ylab="",xlim=c(-6,7),xlab="")
abline(v=0)
...
points(post_mod_gamma,c(21:1),pch=21,bg="black")
